...
@property (nonatomic, strong) UIView *customViewButtons;    ///< 用户自定义布局的按钮view
...
- (UIView *)dzn_customView
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(customViewForEmptyDataSet:)]) {
        ...
        if (view) {
            NSAssert([view isKindOfClass:[UIView class]], @"You must return a valid UIView object for -customViewForEmptyDataSet:");
            return view;
        }
    }
    return nil;
...
- (UIView *)dzn_customViewButtons
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(customViewButtonsForEmptyDataSet:)]) {
        UIView *view = [self.emptyDataSetSource customViewButtonsForEmptyDataSet:self];
        if (view) {
            NSAssert([view isKindOfClass:[UIView class]], @"You must return a valid UIView object for -customViewForEmptyDataSet:");
            return view;
        }
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        if (customView) {
            ...
            if (buttonImage) {
                ...
                view.customViewButtons = [self dzn_customViewButtons];
            }
        }
...
- (UILabel *)titleLabel
{
    if (!_titleLabel)
    {
        ...
        _titleLabel.font = [UIFont fontNormal:15.f];
        _titleLabel.textColor = [UIColor colorWithARGBString:@"#666666"];
...
- (UILabel *)detailLabel
{
    if (!_detailLabel)
    {
        ...
        _detailLabel.font = [UIFont fontBold:12.f];
        _detailLabel.textColor = [UIColor colorWithARGBString:@"#999999"];
...
- (BOOL)canShowCustomButton
{
    return (_customViewButtons && _customViewButtons.superview);
}
...
- (void)setCustomViewButtons:(UIView *)customViewButtons {
    if (!customViewButtons) {
        return;
    }
    
    if (_customViewButtons) {
        [_customViewButtons removeFromSuperview];
        _customViewButtons = nil;
    }
    
    _customViewButtons = customViewButtons;
    _customViewButtons.translatesAutoresizingMaskIntoConstraints = NO;
    [self.contentView addSubview:_customViewButtons];
}
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        // or removes from its superview
            ...
            if([self canShowCustomButton]){
                [subviewStrings addObject:@"customViewButtons"];
                views[[subviewStrings lastObject]] = _customViewButtons;
                
                [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[customViewButtons(>=0)]|"
